...
const routeEntry = (dRoutes, parentRoute) => {
    dRoutes.forEach((value) => {
        ...
        // Trust the name generated by the Vite plugin, otherwise fall back to path
 ...
 * @property {string} [title] - Page title
 * @property {string} [description] - Meta description
     * @property {Record<string, string>} [meta] - Additional meta tags (og:*, twitter:*, keywords, etc)
 ...
 * @returns {{ server: CreateServerSideConfig<A> & { route: string,actions:{...createServerSide.actions }}}}
 */
...
export const handleServerAction = async (req, res) => {
    try {
        ...
        if (isDevelopment || !routeMapsCached) {
    ...
    if (req.file) {
        ...
        if (actionError) {
            ...
            return res.status(500).json({
                success: false,
                error: 'Action failed',
                message: isDevelopment ? actionError.message : 'Internal server error',
                stack: isDevelopment ? actionError.stack : undefined
            })
        }
...
async function loadRouteData(routeConfigs, router, parentChain = true, options = {}) {
    ...
    const generateMetaData = async (config) => {
        ...
        if (getPage && getPage.hasComponent && getPage.component) {
            ...
            let metaResult = {};
            if (typeof metaExport === 'function') {
                const data = routeData[config.route];
                metaResult = metaExport(data?.data || {});
            } else if (metaExport) {
                metaResult = metaExport;
            }

            if (metaResult) {
                Object.assign(router.meta, metaResult);
                
                // Synchronize primary fields for backward compatibility
                if (metaResult.title) router.title = metaResult.title;
                if (metaResult.description) router.description = metaResult.description;
            }
        }
    };

    if (parentChain && routeConfigs.parentChain) {
        for (const config of routeConfigs.parentChain) {
            await generateMetaData(config);
        }
    }
    await generateMetaData(routeConfigs);
...
function buildMetadata(router, serializedState) {
    ...
    const title = meta.title || router.title || 'PawaJS App';
    const description = meta.description || router.description || '';
    
    let tags = `<title>${escapeHtml(title)}</title>`;
    if (description) tags += `\n        <meta name="description" content="${escapeHtml(description)}"/>`;

    // Process all other meta keys
    Object.entries(meta).forEach(([key, value]) => {
        if (key === 'title' || key === 'description') return; // Handled above
        ...
        if (key === 'canonical') {
            ...
            // Standard tags (keywords, robots) and Twitter tags use 'name'
      ...
      router.param = { ...router.param, ...params }
       ...
       // router.param = { ...router.param, ...routeConfigs.params }
        ...
        if (routeConfigs) {
            ...
            if (routeConfigs.meta) Object.assign(router.meta, routeConfigs.meta);
        }
...
export const serverIntializer = async (res, req, html, context, stream, {templateBegin,templateStart, templateEnd}) => {
    ...
    if (!routeConfigs) {
        res.status(404);
        let p = new URL(req.originalUrl, 'http://localhost').pathname;
...
/**
 * CSRF Token provider for static pages
 * @param {import('node:http').IncomingMessage} req
 * @param {import('node:http').ServerResponse} res
 */
export const handleCsrfToken = async (req, res) => {
    const token = crypto.randomBytes(32).toString('hex');
    res.setHeader('Set-Cookie', serialize('XSRF-TOKEN', token, {
        path: '/',
        sameSite: 'lax',
